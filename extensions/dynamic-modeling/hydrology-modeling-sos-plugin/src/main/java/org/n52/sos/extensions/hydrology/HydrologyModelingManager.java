...
/**
 * Main class manager to integrate Hydrologic models (EPANET/SWMM/HEC-RAS) as WaterML Sensors/Observation objects.
 *  
 * @author Alvaro Huarte <ahuarte@tracasa.es>
 */
public class HydrologyModelingManager implements VirtualCapabilitiesExtension 
{
    private static final Logger LOG = LoggerFactory.getLogger(HydrologyModelingManager.class);
    
    /** Default settings file. */
    public static String DEFAULT_SETTINGS_FILE = "dynamic-models/hydrology-settings.xml";
    
    /** Creates a new HydrologyModelingManager using the specified settings file. */
    public HydrologyModelingManager(String settingsFileName)
    {        
        ...
        LOG.info("HydrologyModelingManager registered as DymanicCapabilitiesExtension!");
        
        try
        {
            java.net.URI url = FileUtils.resolveAbsoluteURI(settingsFileName, HydrologyModelingManager.class.getClassLoader());
            if (url!=null) settingsFileName = url.getPath();
            
            LOG.info("Creating HydrologyModelingManager... SettingsFile='"+settingsFileName+"'.");
            
            modelManager = new ModelManager();
            modelManager.loadSettings(settingsFileName);
            modelManager.asynchronouslyPrepareModels();
            
            LOG.info("HydrologyModelingManager successly created!");
        }
        catch (Exception e)
        {
    ...
    private static class LazyHolder implements SosEventListener {
        ...
        private HydrologyModelingManager INSTANCE = null;
        ...
        public void handle(SosEvent event) {
            if (event instanceof ConfiguratorInitializedEvent) {
                try {
                    ...
                    INSTANCE = new HydrologyModelingManager(settingsFileName);
                    INSTANCE.getModelManager();
                }
                catch (Exception e) {
                    HydrologyModelingManager.LOG.error("Error processing Event", e);
                }
            }
        }
    }
    /**
     * Register in the application the LazyHolder of a HydrologyModelingManager singleton instance.
     */
    public static boolean registerSingletonInstance() {
        if (lazyHolder==null) {
            HydrologyModelingManager.LOG.info("Registering HydrologyModelingManager Singleton instance!");
    ...
    /**
     * Returns the HydrologyModelingManager singleton instance of the application.
     */
    public static HydrologyModelingManager getInstance() {
    ...
    /**
     * Returns the related Hydrology workspace managed.
     */
    ...
    /**
     * Returns a ContentCache containing the dynamic capabilities managed by this extension and required for the specified service request.
     */
    ...
    {
      //LOG.info("HydrologyModelingManager::injectDymanicCapabilities()");
    ...
    {
      //LOG.info("HydrologyModelingManager::injectVirtualResponse()");
