...
public class DynamicWritableCache extends InMemoryCacheImpl 
{
    ...
    public void addContentOfDynamicModel(ObservableModel dynamicModel, AbstractServiceRequest<?> request) throws org.n52.sos.exception.CodedException
    {
        ...
        for (ObservableObject theObject : dynamicModel.enumerateObservableObjects(observableContextArgs))
        {
            ...
            if (groupingObjects)
            {
                creatingNewOffering = offeringsMap.get(offering)==null;
                
                if (creatingNewOffering)
                {
                    offeringsMap.put(offering, offering);
                    addOffering(offering);
                    addProcedure(procedure);
            ...
            DateTime maxObsOfferingTime = !creatingNewOffering ? getMaxPhenomenonTimeForOffering(offering) : null;
            ...
            if (theObject.featureOfInterest!=null)
            {
                ...
                if (!creatingNewOffering)
                {
                    SosEnvelope envelope = getEnvelopeForOffering(offering);
                    observedArea.expandToInclude(envelope);
                }
            ...
            if (theObject.attributes.size()>0)
            {
                ...
                {
                    DateTime minObservationTime_i = new DateTime(attribute.timeFrom);
                    DateTime maxObservationTime_i = new DateTime(attribute.timeTo!=null ? attribute.timeTo : new DateTime());
    ...
    private boolean injectDescribeSensorResponseDataOfDynamicModel(AbstractServiceResponse response, ObservableModel dynamicModel, AbstractServiceRequest<?> request) throws org.n52.sos.exception.CodedException
    {
        ...
        for (ObservableObject theObject : dynamicModel.enumerateObservableObjects(observableContextArgs))
        {
            ...
            if (theObject.featureOfInterest!=null)
            {
                ...
                if (groupingObjects)
                {
                    procedureName = DynamicUtils.makeProcedureIdentifier(dynamicModel, theObject);
                    SosProcedureDescription tempProcedure;
                    
                    if ((tempProcedure = procedureMap.get(procedureName))!=null)
                    {
                        tempProcedure.addFeatureOfInterest(DynamicUtils.makeFeatureOfInterestIdentifier(dynamicModel, theObject));
                        continue;
                    }
                }
