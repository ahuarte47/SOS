...
class WonderwareFullObservableObjectCursor extends WonderwareObservableDataCursor<ObservableObject>
{
     ...
     */
    public WonderwareFullObservableObjectCursor(SimpleFeatureModel observableModel, String objectId, String databaseDriverClass, String databaseConnectionUrl, SimpleFeatureSource featureSource, String featureKey, ReferencedEnvelope envelope, List<SimpleObservableAttribute> attributeList)
    {
    ...
    public boolean hasNext() 
    {
        if (featureIterator!=null && featureIterator.hasNext())
        {
            ...
            {
                DateTime startTime = databaseAttribute.timeFrom;
                DateTime finalTime = databaseAttribute.timeTo!=null ? databaseAttribute.timeTo : DateTime.now(startTime.getZone()); //-> now()!
